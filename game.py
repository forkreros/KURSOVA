...
#Отримання тайлів з game.pyxres
def get_tile(tile_x, tile_y):
    return pyxel.tilemaps[0].pget(tile_x, tile_y)

#Створення колізії із стінами та підлогою
...
#Появлення супротивників у певній області
...
#Видалення супротивників
def cleanup_entities(entities):
    for i in range(len(entities) - 1, -1, -1):
        if not entities[i].is_alive:
            del entities[i]

#Класс представляє кулю головного героя
class Bullet:
    ...
    def update(self):
        self.x += BULLET_SPEED * self.direction

        if is_wall(self.x, self.y):
            self.is_active = False

        if self.x < 0 or self.x > scroll_x + 128:
            self.is_active = False

    def draw(self):
        pyxel.rect(self.x, self.y, 2, 2, 8)

#Клас з функціями головного героя
class Player:
    ...
    def update(self):
        ...
        for enemy in enemies:
            enemy.update()
            for bullet in self.bullets:
                if (bullet.x < enemy.x + 8 and bullet.x + 2 > enemy.x and
                        bullet.y < enemy.y + 8 and bullet.y + 2 > enemy.y):
    ...
    def shoot(self):
        bullet_x = self.x + (8 if self.direction > 0 else -2)
        bullet_y = self.y + 4
...
#Клас представляючий рядового супротивника, який падає
class Enemy1:
    def __init__(self, x, y):
        self.x = x
        self.y = y
...
#Клас представляючий рядового супротивника, який не падає
class Enemy2:
    def __init__(self, x, y):
        self.x = x
        self.y = y
...
#Клас представляючий супротивника, що стріляє
class Enemy3:
    def __init__(self, x, y):
        self.x = x
        self.y = y
...
class App:
    def __init__(self):
        ...
        pyxel.init(128, 128, title="PixelFlitz")
        pyxel.load("game.pyxres")

        # Зміна кольору крапок спавну супротивників
    ...
    def draw(self):
        ...
        pyxel.camera()
        pyxel.bltm(0, 0, 0, (scroll_x // 2) % 128, 128, 128, 128)
